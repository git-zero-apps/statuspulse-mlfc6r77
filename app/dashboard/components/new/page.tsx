...
export default function NewComponentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error: insertError } = await supabase.from("components").insert(record);
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        <Link href="/dashboard/components" className="text-sm text-gray-500 hover:text-gray-700 mb-2 inline-flex items-center gap-1">
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
          </svg>
          Back to Components
        </Link>
        <h1 className="text-2xl font-bold text-gray-900 mt-2">Add Component</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card space-y-6">
        ...
        <div>
          <label htmlFor="group_id" className="label">Group Id</label>
          <input id="group_id" name="group_id" type="text" className="input" placeholder="Enter group id" />
        </div>
        <div>
        ...
        <div className="flex items-center gap-3 pt-4 border-t">
          ...
          <Link href="/dashboard/components" className="btn-secondary">Cancel</Link>
        </div>
      </form>
    </div>
  );
